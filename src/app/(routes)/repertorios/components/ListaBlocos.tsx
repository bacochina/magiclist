...
import { XMarkIcon, DocumentDuplicateIcon, QueueListIcon, PlusIcon } from '@heroicons/react/24/outline';
...
export function ListaBlocos({ blocos, onRemoverBloco, onReordenarBlocos, onDuplicarBloco, onAtualizarBlocos }: ListaBlocosProps) {
  ...
  // Função para renderizar o nome da música
  const renderNomeMusica = (musica: Musica) => {
    return musica.nome;
  };
                          ...
                          <ul className="pl-4 space-y-2">
                            {bloco.musicas.map((musica, musicaIndex) => (
                              <li key={musica.id} className="text-sm">
                                {/* Dicas da música (antes da música) */}
                                {musica.dicas && musica.dicas.length > 0 && (
                                  <div className="mb-1">
                                    {musica.dicas.map((dica, dicaIndex) => (
                                      <div key={dicaIndex} className="flex items-center justify-between bg-gray-50 p-2 rounded mb-1">
                                ...
                                <div className="flex items-center">
                                  ...
                                  <span className="text-gray-600">
                                    {renderNomeMusica(musica)}
                                  </span>
                                </div>
                                
                                {/* Formulário para adicionar dica */}
